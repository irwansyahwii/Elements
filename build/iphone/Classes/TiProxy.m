...
//Rare exceptions to indicate a bug in the _elements code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_elements = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_elements]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.elements.module"
